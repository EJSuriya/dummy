...
void addTrain()
{
    ...
    for (auto it :trainList)
    {
    	cout<<"TRAIN NAME : "<< it->getTrainName()<<"    TRAIN ID :"<<it->get_train_id()<<"     DATE :" <<it->getDate()<<endl;
    }
    ...
    for (auto ite :trainList)
    {
    	if(ite->getDate()==date)
    	{
    		cout<<"A TRAIN ALREADY EXISTS WITH THE SAME DATE YOU HAVE PROVIDED !!!"<<endl;
    		goto label;
    	}
    }
    for (auto ite :trainList)
...
void calculateTicketFare()
{
    ...
    string className;
    cout << "enter the class name (AC,sleeper,ladies,sitting) " << endl;
    cin >> className;
    double distance;
    int start = -1;
    int end = -1;
